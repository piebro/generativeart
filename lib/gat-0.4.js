...
gat.CenteredElement = class{
  constructor(elemtTag, size, divObject, settings){
    this.settings = {
      borderAlpha: 0.96,
      ...settings
    }
  ...
  getMiddlePosition(){
    ...
    const width = posRect.width;

    if (width * widthMult <= window.innerHeight) {
...
gat.addSaveAsSVGKey = function(key, drawfunc, namefunc){
  ...
  constructor(){
    ...
    window.addEventListener("keyup", (event) => {
      if (document.activeElement.tagName == "TEXTAREA" || document.activeElement.tagName == "INPUT") {
        ...
        this.typingTimer = setTimeout(redraw, 400);
      }
    });
