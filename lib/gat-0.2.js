...
gat.CenteredElement = class{
  constructor(elemtTag, size, borderAlpha){
...
gat.ParameterHandler = class {
  ...
  addSlider(name, initValue, minValue, maxValue, stepSize){
    ...
    return this.gui.add(this.paramValueMap, name, minValue, maxValue, stepSize)
    .onFinishChange(this.onFinishChangeCalback)
    .listen();
  }

  addButton(name, initValue){
    if(this.gui == undefined){
      this.gui = new dat.GUI();
    }
    this.paramNameList.push(name)
